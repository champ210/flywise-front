...
const DiscoveryHub: React.FC<DiscoveryHubProps> = ({ posts, discoveryData, isLoading, onEarnPoints, onAddComment, onAskAi, onPlanTrip }) => {
    ...
    if (isLoading) {
        return (
            <div className="space-y-6 p-4">
                 <div>
                    <div className="h-6 w-48 rounded bg-slate-200 animate-shimmer mb-3"></div>
                    <div className="flex space-x-4 overflow-x-auto pb-4 custom-scrollbar -mx-4 px-4">
                        {[...Array(3)].map((_, i) => <div key={i} className="flex-shrink-0 w-64 h-40 rounded-lg bg-slate-200 animate-shimmer"></div>)}
                    </div>
                </div>
                <div>
                  <div className="h-6 w-32 rounded bg-slate-200 animate-shimmer mb-4"></div>
                  <TrendCardSkeleton />
                </div>
            </div>
        );
            ...
            {hiddenGemsPosts.length > 0 && (
                <div className="p-4 bg-slate-50 rounded-lg border border-slate-200">
                    ...
                    <div className="space-y-6">
                        {hiddenGemsPosts.map(post => <WandergramPostComponent key={post.id} post={post} onAskAi={onAskAi} onAddComment={onAddComment} onEarnPoints={onEarnPoints} onPlanTrip={onPlanTrip} />)}
                    </div>
                </div>
            )}
            ...
            {recommendedPosts.length > 0 && (
                 <div className="p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                    <h3 className="text-xl font-bold text-slate-800 mb-4 flex items-center"><Icon name="user" className="h-6 w-6 mr-2 text-indigo-500" /> For You</h3>
                     <div className="space-y-6">
                        {recommendedPosts.map(post => <WandergramPostComponent key={post.id} post={post} onAskAi={onAskAi} onAddComment={onAddComment} onEarnPoints={onEarnPoints} onPlanTrip={onPlanTrip} />)}
                    </div>
                </div>
            )}
        </div>
