...
const HostCard: React.FC<HostCardProps> = ({ local, onSelect }) => {
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      aria-label={`View profile for ${local.name}, age ${local.age}, in ${local.location}`}
      className="w-full text-left bg-white rounded-lg shadow-md hover:shadow-xl transition-all duration-300 border border-slate-200 cursor-pointer group flex flex-col"
    >
