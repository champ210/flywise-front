...
const HostDashboard: React.FC<HostDashboardProps> = ({ onSwitchToFinding }) => {
  const [view, setView] = useState<'dashboard' | 'createProfile'>('dashboard');
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 animate-fade-in-up">
      ...
      <div className="mt-8 text-center p-8 bg-slate-50 rounded-lg border-2 border-dashed border-slate-200">
        <h3 className="text-xl font-semibold text-slate-800">Get Started as a Host</h3>
        <p className="mt-2 text-sm text-slate-600">
          You haven't created a host profile yet. Create one now to appear in search results and start receiving requests from travelers.
        </p>
        <button
