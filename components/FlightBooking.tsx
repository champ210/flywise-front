...
const FlightBooking: React.FC<FlightBookingProps> = ({ flight, onClose, onBookingComplete }) => {
  ...
  const handleFinish = () => {
    const confirmation: BookingConfirmation = {
      bookingReference: `FW${Math.random().toString(36).substring(2, 8).toUpperCase()}`,
      flight,
      passenger,
      totalPaid: flight.price + 75.20 // price + mock taxes
    };
    onBookingComplete(confirmation);
  };

  const STEPS = ['Review Flight', 'Passenger Details', 'Payment', 'Confirmation'];
  const basePrice = flight.price;
  const taxes = 75.20; // Mock taxes and fees
  const totalPrice = basePrice + taxes;

  const renderStepContent = () => {
    switch (step) {
      case 1: // Review Flight
        return (
          <div>
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Review Your Flight</h3>
            <div className="bg-slate-100 p-4 rounded-lg border border-slate-200">
                <div className="flex items-center space-x-3">
                    <Icon name="flight" className="h-8 w-8 text-blue-600"/>
                    <div>
                        <p className="font-bold text-slate-800">{flight.airline}</p>
                        <p className="text-sm text-slate-500">{flight.departureAirport} to {flight.arrivalAirport}</p>
                    </div>
                </div>
                <div className="mt-4 text-sm space-y-2">
                    <p><strong>Departure:</strong> {flight.departureTime}</p>
                    <p><strong>Arrival:</strong> {flight.arrivalTime}</p>
                    <p><strong>Duration:</strong> {flight.duration} ({flight.stops > 0 ? `${flight.stops} stop(s)` : 'Direct'})</p>
                </div>
            </div>
            <div className="mt-4 border-t pt-4">
                <h4 className="font-semibold text-slate-700">Price Summary</h4>
                <ul className="text-sm text-slate-600 space-y-1 mt-2">
                    <li className="flex justify-between"><span>Base Fare (1 adult)</span> <span>${basePrice.toLocaleString()}</span></li>
          ...
          <div>
            ...
            <form className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <input type="text" name="firstName" placeholder="First Name" value={passenger.firstName} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
                <input type="text" name="lastName" placeholder="Last Name" value={passenger.lastName} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
              </div>
              <input type="date" name="dateOfBirth" placeholder="Date of Birth" value={passenger.dateOfBirth} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
              <input type="email" name="email" placeholder="Email Address" value={passenger.email} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
              <input type="tel" name="phone" placeholder="Phone Number" value={passenger.phone} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" />
            </form>
          </div>
        );
      case 3: // Payment
      ...
      case 4: // Confirmation
        return (
          <div className="text-center">
            ...
            <p className="mt-2 text-slate-600">Your flight to {flight.arrivalAirport} is booked. A confirmation has been sent to your email.</p>
            <div className="mt-6 text-left bg-slate-100 p-4 rounded-lg text-sm">
                <p><strong>Booking Reference:</strong> <span className="font-mono bg-slate-200 px-2 py-1 rounded">FW{Math.random().toString(36).substring(2, 8).toUpperCase()}</span></p>
                <p className="mt-2"><strong>Passenger:</strong> {passenger.firstName} {passenger.lastName}</p>
      ...
      >
        <header className="p-4 border-b border-slate-200">
          <div className="flex justify-between items-center">
            <h2 id="booking-title" className="text-xl font-bold text-slate-800">
