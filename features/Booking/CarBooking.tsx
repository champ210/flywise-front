...
interface CarBookingProps {
  car: Car;
  onClose: () => void;
  onBookingComplete: (confirmation: CarBookingConfirmation) => void;
}

const CarBooking: React.FC<CarBookingProps> = ({ car, onClose, onBookingComplete }) => {
  const [step, setStep] = useState(1);
  const [driver, setDriver] = useState<PassengerDetails>({
    firstName: '',
    lastName: '',
    dateOfBirth: '',
    email: '',
  ...
  const basePrice = car.pricePerDay * (car.numberOfDays || 1);
  const taxes = basePrice * 0.20; // Mock 20% taxes/fees for cars
  ...
  const renderStepContent = () => {
    switch (step) {
      case 1:
        return (
          <div>
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Review Your Rental Car</h3>
            <div className="flex gap-4">
              <img src={car.imageUrl} alt={`${car.make} ${car.model}`} className="w-24 h-24 rounded-lg object-cover" />
              <div>
                <p className="font-bold text-slate-800">{car.make} {car.model}</p>
                <p className="text-sm text-slate-500">{car.carType} from {car.company}</p>
                <p className="text-sm text-slate-500 mt-1">{car.numberOfDays || 1} day(s) in {car.location}</p>
              </div>
            </div>
            <div className="mt-4 border-t pt-4">
                <h4 className="font-semibold text-slate-700">Price Summary</h4>
                <ul className="text-sm text-slate-600 space-y-1 mt-2">
                    <li className="flex justify-between"><span>Base Price ({car.numberOfDays || 1} days)</span> <span>${basePrice.toLocaleString()}</span></li>
      ...
      case 2:
        return (
          <div>
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Primary Driver Details</h3>
            <form className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <input type="text" name="firstName" placeholder="First Name" value={driver.firstName} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
                <input type="text" name="lastName" placeholder="Last Name" value={driver.lastName} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
              </div>
              <input type="date" name="dateOfBirth" placeholder="Date of Birth" value={driver.dateOfBirth} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
              <input type="email" name="email" placeholder="Email Address" value={driver.email} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" required />
              <input type="tel" name="phone" placeholder="Phone Number" value={driver.phone} onChange={handleInputChange} className="w-full rounded-md border-slate-300 shadow-sm" />
            </form>
          </div>
        );
      case 3:
