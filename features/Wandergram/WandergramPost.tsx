...
const WandergramPostComponent: React.FC<WandergramPostProps> = ({ post, onAskAi, onAddComment, onEarnPoints, onPlanTrip }) => {
        ...
        <div className="bg-white rounded-lg border border-slate-200">
            ...
            <div className="p-3 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <button onClick={handleLike} className="group focus:outline-none" aria-label="Like post">
                     ...
                     <button onClick={() => setIsCommentsOpen(!isCommentsOpen)} className="group focus:outline-none" aria-label="View comments">
                        <Icon name="chat-bubble" className="h-7 w-7 text-slate-700 transition-transform group-hover:scale-110 group-active:scale-95" />
                    </button>
                    <button onClick={() => onAskAi(post)} className="group focus:outline-none" aria-label={`Ask AI about this photo of ${post.location || 'a travel destination'}`}>
                        <Icon name="sparkles" className="h-7 w-7 text-slate-700 transition-transform group-hover:scale-110 group-active:scale-95" />
                    </button>
                    <button onClick={() => onPlanTrip(post)} className="group focus:outline-none" aria-label="Plan a trip like this">
                        <Icon name="planner" className="h-7 w-7 text-slate-700 transition-transform group-hover:scale-110 group-active:scale-95" />
                    </button>
                </div>
                 <button onClick={handleSave} className="group focus:outline-none" aria-label="Save post">
