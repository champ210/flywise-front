...
const ItineraryPlanner: React.FC = () => {
  ...
  const { plannerData, setPlannerData, setChatQuery } = useUIStore();
  ...
  useEffect(() => {
    if (plannerData) {
      setDestination(plannerData.destination);
      setInterests(plannerData.interests);
      setPlannerData(null); // Signal that data has been consumed
    }
  ...
  const handleFindFlights = (plan: ItineraryPlan) => {
    setChatQuery(`Find flights and hotels for a ${plan.itinerary.length}-day trip to ${plan.destination}`);
  };
      ...
      <div className="p-4">
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mt-8">
              ...
              {plan && !isLoading && (
                <>
                  <ItineraryDisplay plan={plan} onSaveTrip={saveTrip} isOffline={isOffline} onFindFlights={handleFindFlights} />
