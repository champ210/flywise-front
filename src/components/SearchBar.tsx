...
import { Flight, Stay, Car, SearchResult } from '@/types';
import { Icon } from './Icon';
import { searchFlightsAmadeus } from '@/src/services/amadeusService';
import { amadeusToFlight, getIataCodeForCity } from '@/src/helpers/flights';
...
const SearchBar: React.FC<SearchBarProps> = ({ onLoading }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    onLoading(true);
    try {
      let results: (Flight | Stay | Car)[] = [];
      if (searchType === SearchType.Flights) {
        // Gather form data here if needed
        const originCity = (document.getElementById('from') as HTMLInputElement)?.value.trim();
        const destinationCity = (document.getElementById('to') as HTMLInputElement)?.value.trim();
        const originLocationCode = await getIataCodeForCity(originCity);
        const destinationLocationCode = await getIataCodeForCity(destinationCity);
        const departureDate = (document.getElementById('depart') as HTMLInputElement)?.value;
        const returnDate = (document.getElementById('return') as HTMLInputElement)?.value;
        const adults = occupancy.passengers || 1; // from props/state
        const travelClass = 'ECONOMY'; // or get from a select input if you add one

        results = await searchFlightsAmadeus({
          originLocationCode,
      ...
      // Add logic for Stays and Cars if needed
