...
const XANO_AUTH_URL = process.env.XANO_AUTH_URL;
...
const apiAuthRequest = async <T>(endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE', body?: unknown): Promise<T> => {
    const headers: HeadersInit = {
        'Content-Type': 'application/json',
    };

    if (!XANO_AUTH_URL) {
        throw new Error("Xano Auth API URL is not configured. Please set XANO_AUTH_URL in your environment.");
    }
    
    const response = await fetch(`${XANO_AUTH_URL}${endpoint}`, {
        method,
        headers,
        body: body ? JSON.stringify(body) : undefined,
    });
...
export const login = async (email: string, password: string): Promise<{ authToken: string }> => {
    const data = await apiAuthRequest<{ authToken: string }>('/auth/login', 'POST', { email, password });
    if (data.authToken) {
...
export const signup = async (name: string, email: string, password: string): Promise<{ authToken: string }> => {
    const data = await apiAuthRequest<{ authToken: string }>('/auth/signup', 'POST', { name, email, password });
    if (data.authToken) {
        setToken(data.authToken);
    }
    return data;
